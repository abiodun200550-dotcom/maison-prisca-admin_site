...
function Monogram({ size = 40 }: { size?: number }) {
  return <Image src="/images/logo-mark.png" alt="Maison Prisca" width={size} height={size} className="shrink-0 object-contain" style={{ width: size, height: size }} />;
}
...
export default function AdminDashboard() {
  ...
  const moveImage = (index: number, direction: -1 | 1) => {
    if (!editing) return;
    const all = [editing.image, ...(editing.gallery || [])].filter(Boolean);
              ...
              <div className="card flex gap-4 p-3 sm:p-4" key={p.$id || p.slug}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      <div className="text-[11px] font-medium text-[var(--muted)]">{p.category} · {p.status}{p.featured ? ' · Featured' : ''}</div>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-[var(--dusk)]/70 p-4">
          <div className="mx-auto my-8 max-w-2xl rounded-[28px] bg-[var(--white)] p-6">
            ...
            <form onSubmit={save} className="mt-7 grid gap-4">
              ...
              <label className="flex items-center gap-3 rounded-xl border border-[var(--line)] bg-[var(--blush)]/20 p-3 text-[13px] font-medium"><input type="checkbox" checked={Boolean(editing.featured)} onChange={e => setEditing({ ...editing, featured: e.target.checked })} /> Feature this piece in the storefront edit</label>
              ...
              <label className="text-[13px] font-medium">Fabric details<input className="input mt-2" placeholder="Silk organza, Adire cotton, Lace" value={editing.fabric || ''} onChange={e => setEditing({ ...editing, fabric: e.target.value })} /></label>
              <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                {[editing.image, ...(editing.gallery || [])].filter(Boolean).map((url, index) => <div className="relative aspect-square overflow-hidden rounded-xl bg-[var(--blush)]/25" key={`${url}-${index}`}><Image src={url} alt={`Product image ${index + 1}`} fill className="object-cover" /><div className="absolute inset-x-2 top-2 flex justify-between"><button type="button" onClick={() => moveImage(index, -1)} className="rounded-full bg-white/90 p-1" aria-label="Move image left"><ChevronLeft size={13} /></button><button type="button" onClick={() => moveImage(index, 1)} className="rounded-full bg-white/90 p-1" aria-label="Move image right"><ChevronRight size={13} /></button></div><button type="button" onClick={() => removeImage(url)} className="absolute bottom-2 right-2 rounded-full bg-white/90 px-2 py-1 text-xs font-medium text-red-700">Remove</button>{index === 0 && <span className="absolute bottom-2 left-2 rounded-full bg-white/90 px-2 py-1 text-[10px] font-medium">Thumbnail</span>}</div>)}
